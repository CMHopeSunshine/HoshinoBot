...
sv_help = '''
1.[#原神猜语音] 开始原神猜语音
2.[#原神猜语音排行榜] 查看本群原神猜语音的排行榜
3.[#原神语音+角色名] 播放指定角色的随机一条语音 
'''.strip()
...
async def get_genshin_voice(bot, ev):
    ...
    if '英' not in name and '韩' not in name and '日' not in name:
        name.insert(0, "中")
    await download_voice(bot, ev)
    path = await get_random_voice(name[1], name[0])
    if not path:
        await bot.finish(ev, f'没有找到{name[0]+name[1]}的语音呢')
